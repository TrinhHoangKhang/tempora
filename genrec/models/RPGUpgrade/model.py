...
class RPGUpgrade(AbstractModel):
    ...
    def forward(self, batch: dict, return_loss: bool = True):
        ...
        Symbols:
            ...
            H : number of prediction heads (= tokenizer.n_digit = D)
            M : number of valid training positions in this batch (label != -100)
            I : number of real items (= n_items - 1, excluding padding id 0)
            D : number of PQ subspaces (= dpq.D)
            v : value dimension per subspace (= dpq.v_dim)

        1. Look up sentence embeddings for each item in the sequence.
        2. Pass through DPQ → STE output feeds into GPT-2.
        3. Apply prediction heads.
        4. Optionally compute per-digit cross-entropy loss:
             - Mean-pool H heads → project to D*v space → split into D sub-queries.
             - Obtain current DPQ code assignments for all items (no gradient).
             - For each subspace d: score sub-query against V[d] → CE vs dynamic code.
             - Loss = mean CE over D subspaces.
        """
        ...
        if return_loss:
            ...
            # Flatten + mask to supervised positions, mean over H heads:
            # final_states reshape : (B*L, H, E)
            # after mask           : (M, H, E)
            # mean over H          : (M, E) → project → (M, D * v_dim)
            selected = final_states.view(
                -1, self.n_pred_head, self.config['n_embd']
            )[label_mask].mean(dim=1)

            query = self.output_proj(selected)                          # (M, D * v_dim)

            # Split into D per-subspace query vectors and L2-normalize.
            # query_chunks: (M, D, v_dim)
            query_chunks = F.normalize(
                query.view(-1, self.dpq.D, self.dpq.v_dim), dim=-1
            )

            # Obtain current DPQ code assignments for all items.
            # Codes are argmax outputs (no gradient), so we use no_grad to avoid
            # building an unnecessary computation graph over the full item table.
            # all_codes: (I, D)  — dynamic assignments at this training step
            with torch.no_grad():
                all_sent = self.sent_emb_table.weight[1:].unsqueeze(0)  # (1, I, d)
                all_codes = self.dpq(all_sent, tau=self.gumbel_tau)['codes'].squeeze(0)  # (I, D)

            # Ground-truth codes for the labeled positions.
            # gt_item_ids : (M,)  0-indexed item IDs
            # gt_codes    : (M, D)
            gt_item_ids = batch['labels'].view(-1)[label_mask] - 1
            gt_codes = all_codes[gt_item_ids]                           # (M, D)

            # Normalize V codebooks for cosine scoring.
            V_norm = F.normalize(self.dpq.V, dim=-1)                    # (D, n_clusters, v_dim)

            # Per-digit logits via einsum: (M, D, n_clusters)
            digit_logits = (
                torch.einsum('mdi,dki->mdk', query_chunks, V_norm)
                / self.temperature
            )

            # Average cross-entropy loss across all D subspaces.
            losses = [
                self.loss_fct(digit_logits[:, d, :], gt_codes[:, d])
                for d in range(self.dpq.D)
            ]
    ...
    def generate(self, batch: dict, n_return_sequences: int = 1, return_loss: bool = False):
        ...
        # Mean over H heads and project to D * v_dim space.
        # states[:, 0]  : (B, H, E)
        # mean(dim=1)   : (B, E)
        # query         : (B, D * v_dim)
        B = states.shape[0]
        query = self.output_proj(states[:, 0].mean(dim=1))             # (B, D * v_dim)

        # Split into D per-subspace query vectors and L2-normalize.
        # query_chunks : (B, D, v_dim)
        query_chunks = F.normalize(
            query.view(B, self.dpq.D, self.dpq.v_dim), dim=-1
        )

        # Obtain current DPQ code assignments for all items (inference, no gradient).
        # all_codes: (I, D)
        with torch.no_grad():
            all_sent = self.sent_emb_table.weight[1:].unsqueeze(0)     # (1, I, d)
            all_codes = self.dpq(all_sent, tau=self.gumbel_tau)['codes'].squeeze(0)  # (I, D)

        # Per-digit log-softmax scores against V codebooks.
        # V_norm       : (D, n_clusters, v_dim)
        # digit_logits : (B, D, n_clusters)
        V_norm = F.normalize(self.dpq.V, dim=-1)                       # (D, n_clusters, v_dim)
        digit_logits = (
            torch.einsum('bdi,dki->bdk', query_chunks, V_norm)
            / self.temperature
        )
        digit_logits = F.log_softmax(digit_logits, dim=-1)             # (B, D, n_clusters)

        # Score each item: gather its per-digit code log-prob, then average across D.
        # all_codes.T          : (D, I) → expand → (B, D, I)
        # item_logits_per_digit: (B, D, I)
        # item_logits          : (B, I)   mean over D
        codes_exp = all_codes.T.unsqueeze(0).expand(B, -1, -1)         # (B, D, I)
        item_logits = digit_logits.gather(
            dim=2, index=codes_exp
