...
class AmazonReviews2018(AbstractDataset):
    ...
    def __init__(self, config: dict):
        super(AmazonReviews2018, self).__init__(config)

        self.category = config['category']
        self._check_available_category()
        self.log(
            f'[DATASET] Amazon Reviews 2018 for category: {self.category}'
        )
    ...
    def _download_raw(self, path: str, type: str = 'reviews') -> str:
        ...
        Args:
            ...
            category (str): The category name (e.g., 'Amazon_Fashion').
            type (str, optional): The type of data to download ('reviews' or 'meta').

        Returns:
            str: The local file path where the downloaded file is saved.
        """

        assert self.config['kcore'] in ['5core', '0core'], "kcore must be '5core' or '0core'."
        
        if type == "reviews":
            if self.config['kcore'] == '5core':
                url = f'https://mcauleylab.ucsd.edu/public_datasets/data/amazon_v2/categoryFilesSmall/{self.category}_5.json.gz'
            else:
                url = f'https://mcauleylab.ucsd.edu/public_datasets/data/amazon_v2/categoryFiles/{self.category}.json.gz'
        elif type == "meta":
            url = f'https://mcauleylab.ucsd.edu/public_datasets/data/amazon_v2/metaFiles2/meta_{self.category}.json.gz'
        else:
            raise ValueError(f"Unsupported type: {type}. Use 'reviews' or 'meta'.")
